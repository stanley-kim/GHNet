...
function getDbArray2($table,$where,$data,$sort,$orderby,$recnum,$p)
{
	global $DB_CONNECT;
	$rcd = db_query('select '.$data.' from '.$table.($where?' where '.getSqlFilter($where):'').' '.($recnum?' limit '.(($p-1)*$recnum).', '.$recnum:''),$DB_CONNECT);
	//return $rcd;
	return 'select '.$data.' from '.$table.($where?' where '.getSqlFilter($where):'').($recnum?' limit '.(($p-1)*$recnum).', '.$recnum:'');
}
function getDbArraySQL($table,$where,$data,$sort,$orderby,$recnum,$p)
{
	global $DB_CONNECT;
	$rcd = db_query('select '.$data.' from '.$table.($where?' where '.getSqlFilter($where):'').' '.($recnum?' limit '.(($p-1)*$recnum).', '.$recnum:''),$DB_CONNECT);
	return 'select '.$data.' from '.$table.($where?' where '.getSqlFilter($where):'').($recnum?' limit '.(($p-1)*$recnum).', '.$recnum:'');
