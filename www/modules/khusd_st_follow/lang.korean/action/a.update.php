...
// 입력값 유효성 체크
$obser = intval($obser);
$obser_pt = intval($obser_pt);
$follow = intval($follow);
$follow_pt = intval($follow_pt);
$follow_report = intval($follow_report);

$date_update = $date['totime'];

// 입력한 데이터로 query 생성
$QKEY = "s_uid, st_id, obser, obser_pt, follow, follow_pt, follow_report, date_update";
$QVAL = "'$s_uid', '$st_id', '$obser', '$obser_pt', '$follow', '$follow_pt', '$follow_report', '$date_update'";
