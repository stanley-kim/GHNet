...
*/
	...
	*/
	...
	$_data_ob_score = 
		...
		.'+ '.$_data_total_imp.' * '.$d['khusd_st_oms']['score']['minor']
		.'+ sc.major * '.$d['khusd_st_oms']['score']['major']
		.'+ sc.cp * '.$d['khusd_st_oms']['score']['cp']
		;
	...
	$_data = 'AVG(charting) AS charting, AVG(dressing) AS dressing, AVG(cp) AS cp, AVG(simple_ext) AS simple_ext, AVG(surgical_ext) AS surgical_ext,'
			.' AVG(minor) AS minor, AVG(major) AS major, AVG(fix) AS fix, AVG(er_call) AS er_call, AVG(st_case) AS st_case, AVG(st_assist) AS st_assist, AVG(imp_1st) AS imp_1st, AVG(imp_2nd) AS imp_2nd'
