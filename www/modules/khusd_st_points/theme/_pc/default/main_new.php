...
$sem_info = getCurrentSemesterInfo();
$s_uid 	= $s_uid  ? $s_uid  : $sem_info['uid'];
...
$_WHERE = $accountQue.'d_occur > '.$year1.sprintf('%02d',$month1).sprintf('%02d',$day1).'000000 and d_occur < '.$year2.sprintf('%02d',$month2).sprintf('%02d',$day2).'240000';
$_NEW_WHERE = 's_uid = '.$s_uid.' and d_occur > '.$year1.sprintf('%02d',$month1).sprintf('%02d',$day1).'000000 and d_occur < '.$year2.sprintf('%02d',$month2).sprintf('%02d',$day2).'240000';
...
*/
$query = 'select mid.id,mbr.memberuid,mbr.name,poh.point as po_h_date,pos.point as po_s_date,poh_tot.point as po_h_tot,pos_tot.point as po_s_tot'
	.' from '.$table['s_mbrdata'].' as mbr'
	.' left join '.$table['s_mbrid'].' as mid on mbr.memberuid=mid.uid'
	.' left join (select my_mbruid, sum(point) as point from '.$table[$m.'point_hospital'].' where '.$_NEW_WHERE.' group by my_mbruid) as poh on mbr.memberuid=poh.my_mbruid'
	.' left join (select my_mbruid, sum(point) as point from '.$table[$m.'point_school'].' where '.$_NEW_WHERE.' group by my_mbruid) as pos on mbr.memberuid=pos.my_mbruid'
	.' left join (select my_mbruid, sum(point) as point from '.$table[$m.'point_hospital'].' where s_uid='.$s_uid.' group by my_mbruid) as poh_tot on mbr.memberuid=poh_tot.my_mbruid'
	.' left join (select my_mbruid, sum(point) as point from '.$table[$m.'point_school'].' where s_uid='.$s_uid.' group by my_mbruid) as pos_tot on mbr.memberuid=pos_tot.my_mbruid'
	.' where 1';

//__debug_print('MODULE: '.$m);	
...
<div id="mbrlist">
	...
	<td><a href="javascript:OpenWindow('<?php echo $g['s']?>/?r=<?php echo $r?>&iframe=Y&m=khusd_st_points&mode=point&mbruid=<?php echo $R['memberuid']?>');" title="상벌점(H)내역"><?php echo number_format($R['po_h_date'])?></a></td>
	...
	<div class="prebox">
		...
		<input type="text" id="d_occur" name="d_occur" size="10" class="input date" />
		<input type="text" name="point" size="5" class="input" />P | 지급사유 : 
