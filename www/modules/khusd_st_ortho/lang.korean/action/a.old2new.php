...
if($FOLLOW && $FOLLOW['uid'] && $FOLLOW['uid'] == $uid)
{
	...
	//$_set = "status = '".$d['khusd_st_ortho']['FOLLOW_STATUS']['DROP']."', date_update = '".$date_update."'";
	$_set = "type = '".$d['khusd_st_ortho']['FOLLOW_TYPE']['NEW']."', date_update = '".$date_update."'";
	$_where = "uid = '".$uid."'";
	...
	$_where = 
		...
		." AND ("
			...
			." OR status = '".$d['khusd_st_ortho']['FOLLOW_STATUS']['COMPLETE']."'"
		.")";
	...
	getLink('/?r=home&m=khusd_st_ortho&mode=follow', '', '구환->신환 변경되었습니다.', '');
}
else
{
