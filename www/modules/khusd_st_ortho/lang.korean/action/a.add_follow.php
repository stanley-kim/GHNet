...
if($GLOBALS['new_pt'])
{
	$type = $d['khusd_st_ortho']['FOLLOW_TYPE']['NEW'];
	//$status = $d['khusd_st_ortho']['FOLLOW']['NEW_FOLLOWING'];
}
else
{
	$type = $d['khusd_st_ortho']['FOLLOW_TYPE']['OLD'];
	//$status = $d['khusd_st_ortho']['FOLLOW']['FOLLOWING'];
}
$status = $d['khusd_st_ortho']['FOLLOW_STATUS']['FOLLOWING'];

// follow_pt 테이블용 상태값
$status_pt = $d['khusd_st_ortho']['FOLLOW_PT']['FOLLOWING'];
...
if($FOLLOW_PT)
{
	...
	$_where = 
		...
		." AND st_id = '".$st_id."'"
		." AND pt_uid = '".$pt_uid."'"
		." AND status = '".$d['khusd_st_ortho']['FOLLOW_STATUS']['FOLLOWING']."'";
	$_sort = 'uid';
	$_orderby = 'ASC';
	...
	if(count($FOLLOW_ARRAY) >= $d['khusd_st_ortho']['FOLLOWER_LIMIT'])
	{
		getLink('', '', '이미 '.$d['khusd_st_ortho']['FOLLOWER_LIMIT'].'명 이상이 팔로우 중인 환자 입니다.', '');
	}
	else if(count($FOLLOW_ARRAY) < $d['khusd_st_ortho']['FOLLOWER_LIMIT'])
	{
	...
	{
		// 기존 팔로우 정보를 업데이트
		$_set = "type = '".$type."', status = '".$status."', date_update = '".$date_update."'";
		$_where = 
			"s_uid = '".$s_uid."'"
...
{
	// 입력한 데이터로 query 생성
	$QKEY = "pt_name, pt_id, dr_room, pf_name, dr_name, status, date_update";
...
if(!$IS_UPDATED)
{
	...
	$_table = $table[$m.'follow'];
	$_where = 
		"pt_uid = '".$FOLLOW_PT_UID."'"
		." AND st_id = '".$st_id."'"
		." AND s_id = '".$s_id."'";
	
	$FOLLOW_UID = getDbCnt($_table, 'uid', $_where);

	if($FOLLOW_UID > 0){
		getLink('', '', '이미 당신이 팔로우 중인 환자 입니다.', '');
	}else{
		$QKEY = "s_uid, pt_uid, st_id, type, status, date_update";
		$QVAL = "'$s_uid', '$FOLLOW_PT_UID', '$st_id', '$type', '$status', '$date_update'";
