...
<div id="mbrlist">
	...
	<div class="sbox">
		...
		<option value="point"<?php if($sort=='point'):?> selected="selected"<?php endif?>>보유상벌점(H)</option>
		<option value="usepoint"<?php if($sort=='usepoint'):?> selected="selected"<?php endif?>>사용상벌점(H)</option>
		<option value="cash"<?php if($sort=='cash'):?> selected="selected"<?php endif?>>보유상벌점(S)</option>
...
<?php if($wideview == 'Y'):?>
	...
	<td><a href="javascript:OpenWindow('<?php echo $g['s']?>/?r=<?php echo $r?>&iframe=Y&m=<?php echo $module?>&front=manager&page=point&price=1&mbruid=<?php echo $R['memberuid']?>');" title="상벌점(H)내역"><?php echo number_format($R['point'])?></a></td>
	<td><a href="javascript:OpenWindow('<?php echo $g['s']?>/?r=<?php echo $r?>&iframe=Y&m=<?php echo $module?>&front=manager&page=point&price=2&mbruid=<?php echo $R['memberuid']?>');" title="상벌점(H)사용내역"><?php echo number_format($R['usepoint'])?></a></td>
...
<?php endif?>
	...
	<div class="prebox">
		...
		<input type="text" name="pwset" size="20" class="input" value="" />
		<input type="button" class="btngray" value="비밀번호초기화" onclick="actQue('tool_pwset');" />
		</div>
		...
		<input type="text" name="price" size="5" class="input" />상벌점(H)(pt) | 지급사유 : 
