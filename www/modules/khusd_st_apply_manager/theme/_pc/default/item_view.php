		...
		<table summary="신청자 리스트입니다.">
			...
			<tbody>
				...
				<?php 
					...
					if($in_limit || $accepted 
						|| (
							$APPLY_INFO['apply_type'] == 'rand' 
							&& $APPLY_INFO['status'] == $d['khusd_st_apply_manager']['apply_info']['OPEN'] 
							&& $APPLIER['status'] != $d['khusd_st_apply_manager']['apply_list']['CANCEL']
							&& $APPLIER['status'] != $d['khusd_st_apply_manager']['apply_list']['OVERAPPLY']
							)
						)
				...
				<tr>
					<td><?php echo $idx++?></td>
					<td class="hand" onclick="getMemberLayer2('<?php echo $APPLIER['mbruid']?>',event);"><?php echo $APPLIER['st_id']?></td>
					<td class="hand" onclick="getMemberLayer2('<?php echo $APPLIER['mbruid']?>',event);"><?php echo $APPLIER['name']?></td>
					<?php if($APPLY_INFO['apply_type'] == 'rand'):?>
					<td>
					...
					<td<?php if($highlight):?> class="highlight"<?php endif?>>
						...
						<?php elseif($APPLIER['date_reg'] < $APPLY_INFO['date_start']):?>
							무효신청<b style="display:none" a="<?=$APPLIER['date_reg']?>" b="<?=$APPLY_INFO['date_start']?>"></b>
