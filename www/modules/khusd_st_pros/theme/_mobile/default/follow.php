<style>
	...
	#pros_follow #followList {padding: 10px 10px 10px 10px;}

	#pros_follow select{height:24px;}
	tr.pt_row{background: #fffff4;}
	.khusd_st tbody td p{font-size:12px;}
	.khusd_st tbody td b{color:#3434ff;}
</style>
		...
		<table class="my_follow">
			...
			<tbody>
				...
				<tr>
					<td>&nbsp;</td>
					<td colspan=3>
					<p>팔로우 상태: <b><?php foreach( $d['khusd_st_pros']['STATUS_OPTIONS'] as $type ):?>
					...
					<?php endforeach?>
						...
						<td colspan=5>
							<p>팔로우 상태: <b><?php foreach( $d['khusd_st_pros']['STATUS_OPTIONS'] as $type ):?>
								...
								<?php endif?>
							<?php endforeach?></b>
							업데이트: <?php echo getDateFormat($FOLLOW['date_update'], 'Y-m-d H:i')?></p>
						</td>
					</tr>
					<?php endforeach?>
				</tbody>
