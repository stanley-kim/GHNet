<div id="pedia_list" class="khusd_st calendar list pedia">

	<table summary="소아치과 ST 신청표 입니다.">
	<caption>
	...
	<th scope="col" class="split"><a href="<?php echo getSortingLink2($g['khusd_st_cnslt_room_manager_st_pedia_list'].$st_date.'&amp;st_timetype='.$st_timetype, 'st_start_time', $om, $order == 'st_start_time')?>">시간</a></th>
	<th scope="col" class="split"><a href="<?php echo getSortingLink2($g['khusd_st_cnslt_room_manager_st_pedia_list'].$st_date.'&amp;st_timetype='.$st_timetype, 'status', $om, $order == 'status')?>">상태</a></th>
	<th scope="col" class="split"><a href="<?php echo getSortingLink2($g['khusd_st_cnslt_room_manager_st_pedia_list'].$st_date.'&amp;st_timetype='.$st_timetype, 'chair_no', $om, $order == 'chair_no')?>">체어</a></th>
	<th scope="col" class="split"><a href="<?php echo getSortingLink2($g['khusd_st_cnslt_room_manager_st_pedia_list'].$st_date.'&amp;st_timetype='.$st_timetype, 'tx_plan', $om, $order == 'tx_plan')?>">진료내용</a></th>
	<th scope="col" class="split"><a href="<?php echo getSortingLink2($g['khusd_st_cnslt_room_manager_st_pedia_list'].$st_date.'&amp;st_timetype='.$st_timetype, 'st_id', $om, $order == 'st_id')?>">학생명</a></th>
	<th scope="col" class="split"><a href="<?php echo getSortingLink2($g['khusd_st_cnslt_room_manager_st_pedia_list'].$st_date.'&amp;st_timetype='.$st_timetype, 'pt_name', $om, $order == 'pt_name')?>">환자명</a></th>
	<th scope="col" class="split"><a href="<?php echo getSortingLink2($g['khusd_st_cnslt_room_manager_st_pedia_list'].$st_date.'&amp;st_timetype='.$st_timetype, 'total_chair', $om, $order == 'total_chair')?>">금주체어신청(총)</th>
	<th scope="col" class="split"><a href="<?php echo getSortingLink2($g['khusd_st_cnslt_room_manager_st_pedia_list'].$st_date.'&amp;st_timetype='.$st_timetype, 'date_reg', $om, $order == 'date_reg')?>">신청시간</a></th>
	<th scope="col" class="split"><a href="<?php echo getSortingLink2($g['khusd_st_cnslt_room_manager_st_pedia_list'].$st_date.'&amp;st_timetype='.$st_timetype, 'date_cancel', $om, $order == 'date_cancel')?>">취소시간</a></th>
	</tr>
	<?php else:?>
	<tr>
	<th scope="col" class="split">No</th>
	...
	<tr>
	<td><?php echo $idx++?></td>
	<td><?php echo $ST_CHAIR['st_start_time']?> ~ <?php echo $ST_CHAIR['st_end_time']?></td>
	<td>
	...
	<form name="applyStChair" method="post" action="<?php echo $g['s']?>/" target="_action_frame_<?php echo $m?>">
		...
		<table summary="소아치과 체어 신청">
		<caption>소아치과 체어 신청</caption>
		<colgroup>
			<col width=150>
			<col>
		</colgroup>
		...
		<tbody>
			...
			<td>
				<select name="st_timetype">
					<?php if(getDateFormat($st_date, 'w') == 6):?>
				...
				<select name="st_time">
					...
					<?php else:?>
					<option value="13301530">13:30~15:30</option>
					<option value="15301730">15:30~17:30</option>
					<?php endif?>
				</select>
			</td>
			</tr>
			...
			<?php
			/*
				<input type="hidden" name="tx_plan" value="" />
					<?php foreach($tx_plan_array as $tx_plan):?>
					<p>
						<input type="checkbox" name="chk_tx_plan" value="<?php echo $tx_plan['id']?>" onchange="updateTxPlanChk();" /><?php echo $tx_plan['name']?>
					</p>
					<?php endforeach?>
			*/
			?>
	...
	//<![CDATA[
	function updateTxPlanChk()
	{
		var chkTxPlan = document.getElementsByName('chk_tx_plan');
		var tx_plan = document.getElementsByName('tx_plan')[0];
		
		tx_plan.value = '';
		
		for(i=0; i < chkTxPlan.length; i++)
		{
			if(chkTxPlan[i].checked)
			{
				if(tx_plan.value != '')	tx_plan.value += ',';
				tx_plan.value += chkTxPlan[i].value;
			}
		}
	}
