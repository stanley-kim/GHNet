<div id="pros_list" class="khusd_st calendar list pros">

	<table summary="보철과 ST 신청표 입니다.">
	<caption>
	...
	<td><?php echo $ST_CHAIR['pt_name']?><br>(<?php echo $ST_CHAIR['pt_id']?>)</td>
	<td><?php echo $ST_CHAIR['dental_formula']?></td>
	<td last="<?php echo $ST_CHAIR['pros_pt_idx'];?>">
		<?php if($ST_CHAIR['pros_pt_idx']):?>
			친환 <?php if($ST_CHAIR[$ST_CHAIR['pros_pt_idx'].'_friendly'] == 'y'):?>O<?php else:?>X<?php endif?>
			/
			<?php echo $ST_CHAIR[$ST_CHAIR['pros_pt_idx'].'_last_tx_date']?>
			/
			<?php echo $tx_plan_array[$ST_CHAIR[$ST_CHAIR['pros_pt_idx'].'_last_tx']]['name']?>
			/
			<?php echo $d['khusd_st_cnslt_room_manager']['pros']['inst']['list'][$ST_CHAIR[$ST_CHAIR['pros_pt_idx'].'_last_inst']]['name']?>
		<?php else:?>
			<?php echo $ST_CHAIR['memo']?>
		<?php endif?>
	</td>
	...
	<form name="applyStChair" method="post" action="<?php echo $g['s']?>/" target="_action_frame_<?php echo $m?>">
		...
		<tbody>
			...
			<td>
				<select name="pros_pt">
					<?php foreach($PROS_PT_ARRAY as $idx => $PROS_PT):?>
					<option value="<?php echo $PROS_PT['id']?>"><?php echo $idx.".".$PROS_PT['pt_name']."//마지막진료:".$tx_plan_array[$PROS_PT['last_tx']]['name']?></option>
					<?php endforeach?>
				</select>
			</td>
			</tr>
			...
			<tr>
			<td colspan="2" class="head">이전 진료 정보는 보철과 점수 업데이트에서 입력된 정보를 자동 반영합니다.</td>
			</tr>
