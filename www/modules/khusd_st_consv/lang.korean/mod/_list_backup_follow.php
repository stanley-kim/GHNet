...
SELECT 
	...
	*/
	...
	$_data_follow_plus_score = '(fw_t.fol_ind_sc + fw_t.fol_endapm_sc + fw_t.fol_endm_sc + fw_t.fol_sur_sc + fw_t.fol_ble_sc + sc.fol_ei*'.$_score['extra-endo-ind'].')';
	...
	$_follow_table = "SELECT st_id"
			...
			.", SUM(if(fw.fw_type='ind' AND fw.status!='d' AND fw.step=2, 1, 0)) AS fol_ind_c"
			...
			." FROM ".$table["khusd_st_followfollow"]." AS fw WHERE fw.s_uid='".$SEMESTER_INFO['uid']."' AND fw.department='consv' GROUP BY st_id";
	...
	$_data = 'AVG(endo_molar) AS endo_molar, AVG(endo_pre) AS endo_pre, AVG(endo_ant) AS endo_ant'
		...
		.', AVG('.$_data_follow_obs_score.') AS fol_sc_obs';
	//__debug_print("%%%%%SELECT $_data FROM $_table WHERE $_where");
